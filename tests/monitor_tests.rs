use qproxy::{
    monitor::{MetricsCollector, GRPC_REQUESTS_TOTAL, HTTP_REQUESTS_TOTAL},
    options::ProxyMode,
};
...
async fn test_metrics_collector() {
    ...
    assert_eq!(
        ...
        qproxy::monitor::ServiceStatus::Up
    );
    assert_eq!(
        health_status.components.recording,
        qproxy::monitor::ServiceStatus::Up
    );
    assert_eq!(
...
async fn test_health_status_updates() {
    ...
    // 更新组件状态
    collector
        .update_component_status("redis", qproxy::monitor::ServiceStatus::Down)
        .await;

    // 验证组件状态已更新
    let updated_status = collector.get_health_status().await;
    assert_eq!(
        updated_status.components.redis,
        qproxy::monitor::ServiceStatus::Down
    );
}
