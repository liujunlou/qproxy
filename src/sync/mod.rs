...
impl SyncService {
    ...
    /// 从peer拉取流量记录并推到回放服务
    pub async fn sync_with_peers(options: &Arc<Options>, client: &Arc<Client>) -> Result<(), Error> {
        if let Some(peer) = &options.peer {
            ...
            let playback_service = PLAYBACK_SERVICE.lock().await;
            // 尝试获取锁, 如果获取失败，则跳过同步
            if !playback_service.as_ref().unwrap().acquire_sync_lock("default", "default").await? {
                return Ok(());
            }
            ...
            // 释放锁
            if let Err(e) = playback_service.as_ref().unwrap().release_sync_lock("default", "default").await {
    ...
    /// 从peer拉取流量记录并推到回放服务
    pub async fn sync_from_peer(client: &Arc<Client>, peer: &PeerOptions, playback_service: Arc<PlaybackService>) -> Result<(), Error> {
